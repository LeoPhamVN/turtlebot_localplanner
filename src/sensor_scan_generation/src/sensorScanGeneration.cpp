...
void OdometryHandler(const nav_msgs::Odometry::ConstPtr& odometry)
{
  ...
  // Set the inverted transform to transformToMapOut
  // transformToMapOut.setOrigin(transform.getOrigin());
  // transformToMapOut.setRotation(transform.getRotation());

  // transformToMapOut.stamp_ = odometry->header.stamp;
  // transformToMapOut.frame_id_ = "turtlebot/kobuki/base_footprint";
  // transformToMapOut.child_frame_id_ = "map";
  // tfBroadcasterPointer->sendTransform(transformToMapOut);
  ...
  transformToMap.frame_id_ = "turtlebot/kobuki/base_footprint";
  // transformToMap.child_frame_id_ = "sensor";
  // tfBroadcasterPointer->sendTransform(transformToMap);

  transformToMap.child_frame_id_ = "vehicle";
  tfBroadcasterPointer->sendTransform(transformToMap);

  // transformToMap.frame_id_ = "turtlebot/kobuki/realsense_depth";
  transformToMap.frame_id_ = "camera_depth_optical_frame";
  transformToMap.child_frame_id_ = "camera";
  tfBroadcasterPointer->sendTransform(transformToMap);
}
...
int main(int argc, char** argv)
{
  ...
  subLaserCloud.subscribe(nh, "/cloud_registered_scan", 1);
